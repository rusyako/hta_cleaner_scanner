...
export default function ManagerDashboard() {
    ...
    useEffect(() => {
        ...
        if (user.role !== 'manager') { router.push('/admin'); return; }
        loadDashboard();
    ...
    const statsCards = [
        { title: 'Всего кабинетов', value: stats?.total_cabinets || 0, icon: BuildingOfficeIcon, bgColor: 'bg-blue-100 dark:bg-blue-900', iconColor: 'text-blue-600 dark:text-blue-400', subtext: 'В системе' },
        { title: 'Уборок сегодня', value: stats?.cleanings_today || 0, icon: CheckCircleIcon, bgColor: 'bg-green-100 dark:bg-green-900', iconColor: 'text-green-600 dark:text-green-400', subtext: 'Завершено' },
        { title: 'Всего отчетов', value: stats?.total_reports || 0, icon: ClockIcon, bgColor: 'bg-purple-100 dark:bg-purple-900', iconColor: 'text-purple-600 dark:text-purple-400', subtext: 'В базе данных' },
    ];
        ...
        <MainLayout>
            <div className="space-y-6">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Панель руководителя</h1>
                        <p className="text-gray-600 dark:text-gray-400 mt-2">Контроль текущей смены и состояния кабинетов</p>
                    </div>
                    <Button variant="outline" onClick={loadDashboard} leftIcon={<ArrowPathIcon className="h-4 w-4" />}>Обновить</Button>
                </div>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <Link href="/cabinets">
                                <div className="p-4 border-2 border-gray-200 dark:border-gray-700 rounded-lg hover:border-blue-500 cursor-pointer transition-colors">
                                    ...
                                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Просмотр статусов всех кабинетов</p>
                                </div>
                            </Link>
                            <Link href="/reports">
                                <div className="p-4 border-2 border-gray-200 dark:border-gray-700 rounded-lg hover:border-green-500 cursor-pointer transition-colors">
                                    <CheckCircleIcon className="h-8 w-8 text-green-600 mb-2" />
                                    <h3 className="font-semibold text-gray-900 dark:text-white">Отчеты</h3>
                                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Просмотр отчетов и фотографий</p>
                                </div>
                            </Link>
                            <Link href="/qr-generator">
                                <div className="p-4 border-2 border-gray-200 dark:border-gray-700 rounded-lg hover:border-yellow-500 cursor-pointer transition-colors">
                                    ...
                                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Создание QR-кодов для кабинетов</p>
                                </div>
                            </Link>
                        </div>
                    </CardContent>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    <Card>
                        <CardHeader><CardTitle>Последние отчеты</CardTitle></CardHeader>
                        <CardContent className="space-y-3">
                            {recentReports.length === 0 && <Alert variant="info">Пока нет отчетов за смену.</Alert>}
                                ...
                                <div key={report.id} className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900/40">
                                    <div className="flex items-start justify-between gap-3">
                                        <div>
                                            ...
                                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{new Date(report.timestamp).toLocaleString('ru-RU')}</p>
                                        </div>
                                    ...
                                    <div className="mt-3 flex items-center gap-3 text-sm text-gray-600 dark:text-gray-300">
                                        ...
                                        <span className="flex items-center gap-1"><PhotoIcon className="h-4 w-4" />{report.photos.length}</span>
                                    </div>
                                </div>
                            ))}
                            {recentReports.length > 0 && (
                                <div className="pt-2">
                                    <Link href="/reports" className="text-sm font-medium text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300">Открыть все отчеты</Link>
                                </div>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardHeader><CardTitle>Требуют внимания</CardTitle></CardHeader>
                        <CardContent className="space-y-3">
                            {attentionCabinets.length === 0 && <Alert variant="success">Сейчас все кабинеты в хорошем состоянии.</Alert>}
                                ...
                                <div key={cabinet.cabinet_number} className="rounded-2xl border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-800 dark:bg-yellow-900/20">
                                    <div className="flex items-start justify-between gap-3">
                                        <div>
                                            ...
                                            <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">Последняя уборка: {cabinet.last_cleaned ? new Date(cabinet.last_cleaned).toLocaleString('ru-RU') : 'нет данных'}</p>
                                        </div>
                                        <Badge variant="warning">Требует уборки</Badge>
                                    </div>
                                    <p className="mt-3 text-sm text-gray-600 dark:text-gray-300">Ответственный: {cabinet.cleaner_name || 'не назначен'}</p>
                                </div>
                            ))}
                            {attentionCabinets.length > 0 && (
                                <div className="pt-2">
                                    <Link href="/cabinets" className="text-sm font-medium text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300">Открыть все кабинеты</Link>
                                </div>
                            )}
                        </CardContent>
                    </Card>
